...
public class EnderPearlTimer extends PlayerTimer implements Listener {
	...
	public void onExpire(UUID userUUID) {
		...
		if (player != null) {
			player.sendMessage(ChatColor.GREEN + "Your " + getDisplayName() + ChatColor.GREEN + " timer has expired.");
		}
	}
    ...
    public void onProjectileLaunch(ProjectileLaunchEvent event) {
        ...
        if (projectile instanceof EnderPearl) {
            ...
            if (source instanceof Player) {
                ...
                if (remaining > 0L) {
                    shooter.sendMessage(ChatColor.RED + "You are still on " + ChatColor.YELLOW.toString() + ChatColor.BOLD + getName() + ChatColor.RED + " cooldown for another " + ChatColor.BOLD
		...
		public void run() {
			...
			if (stack != null && stack.getType() == Material.ENDER_PEARL) {
				...
				if (remaining > 0L) {
					...
					item.c(ChatColor.YELLOW + "Enderpearl Cooldown" + ChatColor.GRAY + ": " + ChatColor.WHITE + DurationFormatter.getRemaining(remaining, true, true));
