...
import us.hcrealms.hcf.faction.event.FactionChatEvent;
import us.hcrealms.hcf.faction.struct.ChatChannel;
...
@EventHandler(ignoreCancelled=true, priority=EventPriority.HIGHEST)
  public void onPlayerChat(AsyncPlayerChatEvent event)
  {
	    ...
	    if ((message.toLowerCase().contains(word)))
	    {
	      ...
	      for (int i = 0; i < j; i++)
	      {
	        ...
	        if (on.hasPermission("command.mod")) {
	          on.sendMessage(ChatColor.DARK_GRAY + "[" + ChatColor.DARK_RED.toString() + ChatColor.BOLD + "BLOCKED" + ChatColor.DARK_GRAY + "] " + ChatColor.RED + player.getDisplayName() + ChatColor.GRAY + " attempted to say " + ChatColor.RED + message);
	        }
	      }
	      event.setCancelled(true);
	      return;
    ...
    console.sendMessage(ChatColor.GOLD + "[" + tag + ChatColor.GOLD + "] " + displayName + ChatColor.WHITE + ": " + ChatColor.WHITE + message);
    for (Player recipient : event.getRecipients())
    {
    	...
    	{
  	      recipient.sendMessage(ChatColor.GOLD + "[§c*§6] §r" + displayName + ChatColor.WHITE + ": " + ChatColor.WHITE + message);
        }
      }
  }
  ...
  static
  {
	blocked = Arrays.asList("hitler", "kys", "nigga", "kill yourself", "nigger", "rip off", "shit staff", "shit server", "gay server", "kys", "leaked", "ddos", "kill yourself", "shit owner", "join my server", "new hcf server", "server is shit", "server is crap", "bad server", "kult", "this is shit", "customkkk", "niggerfaggot", "faggot", "heil");  
