	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0 || args.length > 2) {
            sender.sendMessage(
                    "§9§m--------------------------------");
            sender.sendMessage(
            		"§c/reboot start <duration> - Starts the reboot timer.");
            sender.sendMessage(
            		"§c/reboot end - Ends the current reboot timer.");
            sender.sendMessage(
                    "§9§m--------------------------------");
			return true;
		}
		...
		if (args.length == 2) {
			if (args[0].equalsIgnoreCase("start")) {
				...
				Bukkit.broadcastMessage(ChatColor.GRAY + "The server is " + ChatColor.DARK_RED.toString() + ChatColor.BOLD + "rebooting " + ChatColor.GRAY + "in "
						+ DurationFormatUtils.formatDurationWords(duration, true, true) + ".");
			} else {
				sender.sendMessage(ChatColor.RED + "Unknown sub-command!");
				return true;
