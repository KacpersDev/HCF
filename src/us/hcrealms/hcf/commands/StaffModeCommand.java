...
import us.hcrealms.hcf.staffmode.StaffItems;
...
public class StaffModeCommand implements Listener, CommandExecutor {
	...
	public static boolean enterMod(final Player p) {
		...
		p.sendMessage("§7You have §aenabled §7your §3§lStaff Mode");
		return true;
	}
	...
	public static boolean leaveMod(final Player p) {
		...
		p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&7You have &cdisabled &7your &3&lStaff Mode"));
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("staffmode")) {
			...
			if (modMode.contains(t)) {
				leaveMod(t);
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&7You have &cdisabled &7" + t.getName() + "'s &3&lStaff Mode"));
				return true;
			}
			enterMod(t);
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&7You have &aenabled &7" + t.getName() + "'s &3&lStaff Mode"));
			return true;
		}
		return false;
	}
