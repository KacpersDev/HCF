...
public LFFCommand(Base plugin) {
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        if (Cooldowns.isOnCooldown("lff_cooldown", (Player)sender)) {
            sender.sendMessage(ChatColor.RED + "You cannot use this for another " + ChatColor.RED + ChatColor.BOLD.toString() + Base.getRemaining(Cooldowns.getCooldownForPlayerLong("lff_cooldown", (Player)sender), true));
            return true;
        }
        Bukkit.broadcastMessage(ChatColor.DARK_GRAY + ChatColor.STRIKETHROUGH.toString() + "------------------------------");
        Bukkit.broadcastMessage(ChatColor.GOLD + ChatColor.BOLD.toString() + sender.getName() + ChatColor.YELLOW + " is looking for a " + ChatColor.GREEN + ChatColor.BOLD.toString() + "FACTION" + ChatColor.YELLOW + ".");
        Bukkit.broadcastMessage(ChatColor.DARK_GRAY + ChatColor.STRIKETHROUGH.toString() + "------------------------------");
