package us.hcrealms.hcf.staffmode;
...
import us.hcrealms.hcf.util.TimeUtils;
	...
	public static void inspector(Player player, Player target) {
		...
		Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
			public void run() {
				...
				compassMeta.setLore(translateFromArray(Arrays.asList(new String[] { "&eWorld&7: &e" + player.getWorld().getName(),"&eCoords", "  &eX&7: &c" + target.getLocation().getBlockX(), "  &eY&7: &c" + target.getLocation().getBlockY(), "  &eZ&7: &c" + target.getLocation().getBlockZ() })));
