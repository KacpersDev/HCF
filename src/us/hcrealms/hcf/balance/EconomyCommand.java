package us.hcrealms.hcf.balance;
...
import us.hcrealms.hcf.BaseConstants;
...
public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
  {
    ...
    {
      sender.sendMessage(ChatColor.GRAY + (sender.equals(target) ? "Balance: " : new StringBuilder(String.valueOf(target.getName())).append("'s Balance: ").toString()) + ChatColor.AQUA + '$' + balance);
      return true;
    }
    ...
    {
      sender.sendMessage(ChatColor.AQUA + (sender.equals(target) ? "Your balance" : new StringBuilder("Balance of ").append(target.getName()).toString()) + " is " + ChatColor.WHITE + '$' + balance + ChatColor.AQUA + '.');
      return true;
    }
    if (args.length < 3)
    {
    ...
    sender.sendMessage(ChatColor.GRAY + "Set balance of " + ChatColor.AQUA + target.getName() + ChatColor.GRAY + " to " + ChatColor.GREEN + '$' + JavaUtils.format(Integer.valueOf(newBalance)) + '.');
    return true;
  }
