package us.hcrealms.hcf.faction.struct;
...
public enum ChatChannel {
    ...
    public String getRawFormat(final Player player) {
        switch(this) {
            case FACTION: {
                return ChatColor.DARK_AQUA + "(" + "Team" + ChatColor.DARK_AQUA + ") " + player.getName() + ": " + ChatColor.YELLOW + "%2$s";
            }
            case ALLIANCE: {
            	return ChatColor.BLUE + "(" + "Ally" + ChatColor.BLUE + ") " + player.getName() + ": " + ChatColor.YELLOW + "%2$s";
            }
            default: {
