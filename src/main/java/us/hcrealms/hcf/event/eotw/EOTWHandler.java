package us.hcrealms.hcf.event.eotw;
    ...
    public static final class EotwRunnable extends BukkitRunnable {
        ...
        public void run() {
            ...
            if(!this.hasInformedStarted && elapsedSeconds >= 0) {
                ...
                Bukkit.broadcastMessage(ChatColor.DARK_RED.toString() + ChatColor.BOLD + "EOTW" + ChatColor.RED + " has began. Border will decrease by " + BORDER_DECREASE_AMOUNT + " blocks every " + BORDER_DECREASE_TIME_WORDS + " until at " + BORDER_DECREASE_MINIMUM + " blocks.");
                return;
            }
            if(elapsedMillis < 0L && elapsedMillis >= -EOTW_WARMUP_WAIT_MILLIS) {
                Bukkit.broadcastMessage(ChatColor.DARK_RED.toString() + ChatColor.BOLD + "EOTW" + ChatColor.RED + " is starting in " + Base.getRemaining(Math.abs(elapsedMillis), true, false) + '.');
                return;
            }
            ...
            if(elapsedSeconds % BORDER_DECREASE_TIME_SECONDS == 0) {
                ...
                data.saveConfig();
                Bukkit.broadcastMessage(ChatColor.RED + "Border has been decreased to " + ChatColor.YELLOW + newBorderSize + ChatColor.RED + " blocks.");
            } else if(elapsedSeconds % (TimeUnit.MINUTES.toSeconds(5)) == 0) {
                Bukkit.broadcastMessage(ChatColor.RED + "Border decreasing to " + ChatColor.YELLOW + newBorderSize + ChatColor.RED + " blocks in " + ChatColor.YELLOW + BORDER_DECREASE_TIME_ALERT_WORDS + ChatColor.RED + '.');
            }
