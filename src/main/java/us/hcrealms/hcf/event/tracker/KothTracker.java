  ...
  implements EventTracker
{
  ...
  public void tick(EventTimer eventTimer, EventFaction eventFaction)
  {
    ...
    if ((remainingSeconds > 0) && (remainingSeconds % 30 == 0)) {
      Bukkit.broadcastMessage(ChatColor.GOLD + "[" + "KingOfTheHill" + "] " + ChatColor.YELLOW + captureZone.getDisplayName() + ChatColor.GOLD + " is trying to be controlled.");
      Bukkit.broadcastMessage(ChatColor.GOLD + " - Time Left: " + ChatColor.BLUE + DateTimeFormats.KOTH_FORMAT.format(remainingMillis));
    }
  }
  
  public void onContest(EventFaction eventFaction, EventTimer eventTimer)
  {
    Bukkit.broadcastMessage(ChatColor.GOLD + "[" + "KingOfTheHill" + "] " + ChatColor.YELLOW + eventFaction.getName() + ChatColor.GOLD + " can now be contested. ");
    Bukkit.broadcastMessage(ChatColor.GOLD + " - Time Left: " + ChatColor.BLUE + DateTimeFormats.KOTH_FORMAT.format(eventTimer.getRemaining()));
  }
  
  public boolean onControlTake(Player player, CaptureZone captureZone)
  {
    player.sendMessage(ChatColor.GOLD + "You are now in control of " + ChatColor.YELLOW + captureZone.getDisplayName() + ChatColor.GOLD + '.');
    return true;
  }
  
  public boolean onControlLoss(Player player, CaptureZone captureZone, EventFaction eventFaction)
  {
    player.sendMessage(ChatColor.GOLD + "You are no longer in control of " + ChatColor.YELLOW + captureZone.getDisplayName() + ChatColor.GOLD + '.');
    long remainingMillis = captureZone.getRemainingCaptureMillis();
    if ((remainingMillis > 0L) && (captureZone.getDefaultCaptureMillis() - remainingMillis > MINIMUM_CONTROL_TIME_ANNOUNCE)) {
      Bukkit.broadcastMessage(ChatColor.GOLD + "[" + "KingOfTheHill" + "] " + ChatColor.YELLOW + player.getName() + ChatColor.GOLD + " has lost control of " + ChatColor.YELLOW + captureZone.getDisplayName() + ChatColor.GOLD + '.');
      Bukkit.broadcastMessage(ChatColor.GOLD + " - Time Left: " + ChatColor.BLUE + DateTimeFormats.KOTH_FORMAT.format(captureZone.getRemainingCaptureMillis()));
    }
    return true;
  }
