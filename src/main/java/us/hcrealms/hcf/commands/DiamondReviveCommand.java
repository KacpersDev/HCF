...
public class DiamondReviveCommand implements CommandExecutor {
	private final Base plugin;

	public DiamondReviveCommand(final Base plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(final CommandSender sender, final Command command, final String label,
		...
		if (Cooldowns.isOnCooldown("diamond_cooldown", player)) {
			sender.sendMessage("§cYou cannot do this for another §l"
					+ Cooldowns.getCooldownForPlayerInt("diamond_cooldown", player) / 60 + " §cminutes.");
			return true;
		}
		...
		Bukkit.broadcastMessage(ChatColor.GOLD + sender.getName() + ChatColor.YELLOW + " has revived " + ChatColor.GOLD + target.getName() + ChatColor.YELLOW + " with their Diamond rank.");
		Cooldowns.addCooldown("diamond_cooldown", player, 3600);
