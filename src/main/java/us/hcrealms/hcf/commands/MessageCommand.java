  ...
  implements CommandExecutor
{
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
  {
    ...
    if (sender instanceof Player)
    {
      ...
      {
        sender.sendMessage(MessageEvent.format(otherprefix + Bukkit.getPlayer(args[0]).getName() + "&7 has their messages disabled."));
      }
      else
      {
        String msgto = MessageEvent.format("&7(&fme &7to " + otherprefix + Bukkit.getPlayer(args[0]).getName() + "&7)&7 " + MessageEvent.toString(args, 1));
        sender.sendMessage(msgto);
        
        String msgrecieve = MessageEvent.format("&7(" + senderprefix + sender.getName() + "&7 to &fme&7) &7" + MessageEvent.toString(args, 1));
