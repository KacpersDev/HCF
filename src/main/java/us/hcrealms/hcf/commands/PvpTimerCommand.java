...
public class PvpTimerCommand implements CommandExecutor, TabCompleter {
	...
	private void printUsage(CommandSender sender, String label, PvPTimerProtection pvpTimer) {
		sender.sendMessage(ChatColor.RED + "/pvp enable" + ChatColor.RED + " - " + ChatColor.RED + "Remove your PvP Protection.");
		sender.sendMessage(ChatColor.RED + "/pvp time" + ChatColor.RED + " - " + ChatColor.RED + "Check the remaining for your PvP Protection.");
		sender.sendMessage(ChatColor.RED + "/lives" + ChatColor.RED + " - " + ChatColor.RED + "Info about lives and deathbans.");
	}
}
