  ...
  implements CommandExecutor
{
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
  {
      ...
      {
        sender.sendMessage(MessageEvent.format("&7You are currently in a conversation with " + otherprefix + ((Player)MessageCommand.reply.get(sender)).getName()));
      }
      ...
      {
        sender.sendMessage(MessageEvent.format(otherprefix + ((Player)MessageCommand.reply.get(sender)).getName() + " has their messages disabled."));
      }
      else
      {
        String msgto = MessageEvent.format("&7(&fme &7to " + otherprefix + MessageCommand.reply.get(sender).getName() + "&7)&7 " + MessageEvent.toString(args, 0));
        sender.sendMessage(msgto);
        String msgget = MessageEvent.format("&7(" + senderprefix + sender.getName() + "&7 to &fme&7)&7 " + MessageEvent.toString(args, 0));
