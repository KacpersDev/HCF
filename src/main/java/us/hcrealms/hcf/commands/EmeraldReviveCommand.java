...
public class EmeraldReviveCommand implements CommandExecutor {
	private final Base plugin;

	public EmeraldReviveCommand(final Base plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(final CommandSender sender, final Command command, final String label,
		...
		if (Cooldowns.isOnCooldown("emerald_cooldown", player)) {
			sender.sendMessage("§cYou cannot do this for another §l"
					+ Cooldowns.getCooldownForPlayerInt("emerald_cooldown", player) / 60 + " §cminutes.");
			return true;
		}
		...
		Bukkit.broadcastMessage(ChatColor.GOLD + sender.getName() + ChatColor.YELLOW + " has revived " + ChatColor.GOLD + target.getName() + ChatColor.YELLOW + " with their Emerald rank.");
		Cooldowns.addCooldown("emerald_cooldown", player, 3600);
