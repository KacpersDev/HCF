package us.hcrealms.hcf.classes.archer;
  ...
  implements Listener
{
  ...
  public void onPlayerClickFeather(PlayerInteractEvent e)
  {
    ...
    if ((this.plugin.getPvpClassManager().getEquippedClass(p) != null) && (this.plugin.getPvpClassManager().getEquippedClass(p).equals(this)) && (p.getItemInHand().getType() == Material.FEATHER))
    {
      ...
      {
        p.sendMessage(ChatColor.RED + "You cannot use this for another " + ChatColor.BOLD.toString() + Base.getRemaining(Cooldowns.getCooldownForPlayerLong("archer_jump_cooldown", p), true) + ChatColor.RED + '.');
        e.setCancelled(true);
        return;
      }
  ...
  public void onPlayerClickSugar(PlayerInteractEvent e)
  {
    ...
    if ((this.plugin.getPvpClassManager().getEquippedClass(p) != null) && (this.plugin.getPvpClassManager().getEquippedClass(p).equals(this)) && (p.getItemInHand().getType() == Material.SUGAR))
    {
      ...
      {
        p.sendMessage(ChatColor.RED + "You cannot use this for another " + ChatColor.BOLD.toString() + Base.getRemaining(Cooldowns.getCooldownForPlayerLong("archer_speed_cooldown", p), true) + ChatColor.RED + '.');
        e.setCancelled(true);
        return;
      }
    ...
    if (((entity instanceof Player)) && ((damager instanceof Arrow)))
    {
      ...
      if ((source instanceof Player))
      {
        ...
        if (this.plugin.getTimerManager().archerTimer.getRemaining((Player)entity) == 0L)
        {
          ...
          shooter.sendMessage(ChatColor.YELLOW + "[" + ChatColor.BLUE + "Arrow Range" + ChatColor.YELLOW + "(" + ChatColor.RED + round + ChatColor.YELLOW + ")]" + ChatColor.GOLD + " Marked " + damaged.getName() + " for 10 seconds " + ChatColor.BLUE.toString() + ChatColor.BOLD + "(" + event.getDamage() + ")");
          damaged.sendMessage(ChatColor.RED.toString() + ChatColor.BOLD + "Archer Marked! " + ChatColor.YELLOW + "You have been Archer Tagged, any damage you take by a player will be increased by 25%");
        }
      }
    }
  }
